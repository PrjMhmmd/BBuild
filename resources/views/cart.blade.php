...
<html>
    <head>
        ...
        <title>BBuild.si | Keranjang</title>
    ...
    <body>
        ...
        <section id="cart">
            <section id="menu">
              <h2>KERANJANG</h2>
                @if ($orders->status="pending" && $totalHarga!=0)
                    @foreach($orders as $order)
                        @foreach($order->detail_orders as $detailOrder)
                            <div class="produk">

                                <div>
                                    @if($detailOrder->produk->image)
                                        <img class="img" src="{{ asset('../storage/'. $detailOrder->produk->image) }}">
                                    @else
                                ...
                                <div class="text">
                                    <h3>{{ $detailOrder->produk->nama_barang }}</h3>
                                    <h4 id="jumlah">Jumlah: {{ $detailOrder->jumlah }}</h4>
                                    <p style="text-align: justify;">{{ $detailOrder->produk->deskripsi_barang }}</p>
                                </div>

                                <div class="remove">
                                    <form action="/cart/remove/{{ $detailOrder->id }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button class="hapus" type="submit">X</button>
                                    </form>
                                </div>
                    ...
                    <div class="beli">
                        ...
                        <form action="/cart/pengiriman/redirect" method="POST">
                        @csrf
                          <h2 class="total">Total Harga: IDR {{ $totalHarga }}</h2>
                          <button class="button" type="submit"><h3>BELI</h3></button>
                        </form>
